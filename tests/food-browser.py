...
with sync_playwright() as w:
 ...
 def snap(name):p.screenshot(path=str(OUT/name),full_page=True)
 p.on('pageerror',lambda e:errors.append(str(e)))
 p.on('dialog',lambda d:d.accept())
 goto('/recipes/')
 ok('Six source-backed recipes load',p.locator('[data-recipe-grid] .food-card').count()==6)
 p.locator('[data-recipe-search]').fill('tropical');ok('Recipe search filters',p.locator('[data-recipe-grid] .food-card').count()==1)
 p.locator('[data-recipe-search]').fill('');p.locator('[data-recipe-category]').select_option('Dinner');ok('Meal filter works',p.locator('[data-recipe-grid] .food-card').count()==2)
 p.locator('[data-recipe-category]').select_option('');snap('recipes-desktop.png')
 ...
 if OFFLINE:capture_storage()
 goto('/meal-planner/manage/');p.locator('[data-copy-plan] [name=name]').fill('QA copied plan');p.locator('[data-copy-plan] button').click();follow()
 ok('Plan duplicated without duplicating food logs',len(persisted()['plans'])==2 and len(persisted()['logs'])==1)
 if OFFLINE:capture_storage()
 goto('/programmes/core/');p.locator('[data-system-base] input[value=arms]').check();p.locator('[data-system-base] input[value=shoulders]').check()
 ...
 for route in ['/','/recipes/','/meal-planner/?date='+date,'/shopping-list/','/programmes/','/courses/','/shop/','/account/']:
  ...
  if route=='/recipes/':snap('recipes-mobile.png')
