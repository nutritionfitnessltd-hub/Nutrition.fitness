...
for(let attempt=1;attempt<=18;attempt++){
 try{
  ...
  if(imported){const detail=await get(`recipes/${imported.id}/`);if(!detail.includes(`data-recipe-locked="${imported.id}"`)||!detail.includes('Account access coming soon')||/data-recipe-detail|data-method|recipeInstructions|recipeIngredient/.test(detail))throw new Error('Restricted recipe preview is missing or leaks cooking content.');if(imported.image){const photo=await fetch(`${origin}${imported.image}?verify=${stamp}`,{signal:AbortSignal.timeout(15000)});if(!photo.ok||digest(Buffer.from(await photo.arrayBuffer()))!==digest(await readFile(new URL('../public'+imported.image,import.meta.url))))throw new Error('Imported recipe photograph does not match the tested asset.');}}
