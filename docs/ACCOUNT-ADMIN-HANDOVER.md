...
**Database provisioning is complete; live website activation is pending.** On the owner's explicit instruction, Castle Grove (`bedcwnehhlygyuuvltbv`) was paused and confirmed INACTIVE. The dedicated **Nutrition.Fitness** project (`fyzapcqcsfpknblsixku`) was created in the **Nutrition dot fitness** organisation (`fvsmsnvlfcmsnxljfbjm`), London (`eu-west-2`), at the confirmed **$0/month** project cost. All six prepared migrations are installed on its PostgreSQL 17 database. The original 100 public recipes are seeded. All 499 private recipes are imported and verified: 481 published, 18 held for review, and 39 with unknown nutrition. The final hosted checks passed for all 13 tables, all 15 protected functions, all recipe identifiers and stored source/content hashes. The intended first administrator is `james@nutrition.fitness`. See the [provisioning record](auth-activation/provisioning-status.json), [hosted database verification](auth-activation/database-verification.json) and [activation instructions](auth-activation/AUTH-CONFIG-HANDOFF.md).
...
The selected dedicated Nutrition.Fitness project is `fyzapcqcsfpknblsixku`. Its six migrations are already installed. Do not rerun the initial migrations on that project or apply them to Castle Grove, Love Stories Events or Tan Time. The filename-order instructions below are for a fresh installation. Hosted migration records use the application timestamps generated by the Supabase connector; match the installed names and the source checksums in the provisioning record before any later CLI reconciliation.
...
For this newly created Free project, configure custom SMTP before editing the email templates or testing the website's code-entry flow. Supabase disabled template customization for new Free projects that use the default sender on 3 June 2026; see the [official change notice](https://supabase.com/changelog/46599-changes-to-email-template-customisation-on-free-tier).
