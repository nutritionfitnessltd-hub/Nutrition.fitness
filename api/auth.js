...
import {body,reply,fail,session,setSession,clearSession,platform,platformConfigured,passwordConfigured,passwordLoginConfigured,publicSignupConfigured,provisionedAccountsOnly,password,verifiedUser,membership,setRecovery,consumeRecovery,verifyBot,email,text,HttpError} from '../server/platform.mjs';
...
function providerSecurity(input,env){
 const site=!!env.TURNSTILE_SITE_KEY,secret=!!env.TURNSTILE_SECRET_KEY;
 if(!site&&!secret)return {};
 if(!site||!secret||env.NUFI_AUTH_CAPTCHA_READY!=='true')throw new HttpError(503,'The security check is not configured completely.');
 ...
 return {gotrue_meta_security:{captcha_token:input.botToken}};
}
...
export function createAuthHandler({env=process.env,fetcher=fetch}={}){return async(req,res)=>{try{
 ...
 const passwordReady=passwordLoginConfigured(env),signupReady=publicSignupConfigured(env),recoveryReady=passwordConfigured(env);
 if(req.method==='GET'){
  ...
  return reply(res,200,{configured:otpReady,passwordConfigured:loginReady,signupConfigured:!provisioned&&(otpReady||signupReady),recoveryConfigured:!provisioned&&recoveryReady,passwordChangeConfigured:recoveryReady,provisionedAccountsOnly:provisioned,captchaRequired:!!(env.TURNSTILE_SITE_KEY||env.TURNSTILE_SECRET_KEY),turnstileSiteKey:env.TURNSTILE_SITE_KEY||null,...(connection?{connection}: {})});
 }
 ...
 if(input.action==='login-password'&&!passwordReady)throw new HttpError(503,'Password sign-in has not been configured and tested yet.');
 if(input.action==='signup-password'&&!signupReady)throw new HttpError(503,'Free account registration is not available yet.');
 if(['request-reset','verify-recovery','reset-password'].includes(input.action)&&!recoveryReady)throw new HttpError(503,'Password recovery is not available yet.');
 ...
 const address=email(input.email),security=providerSecurity(input,env);
 if(input.action==='send-code'){
  ...
  try{await api('/auth/v1/otp',{method:'POST',payload:{email:address,create_user:input.flow==='signup',...(data?{data}:{}),...security}});}catch(e){if(!['otp_disabled','user_not_found'].includes(e.upstreamCode))throw e;}
 ...
 if(input.action==='signup-password'){
  ...
  try{result=await api('/auth/v1/signup',{method:'POST',payload:{email:address,password:nextPassword,data,...security}});}catch(e){if(!['user_already_exists','email_exists'].includes(e.upstreamCode))providerError(e,'The account could not be created. Check the security check and use a stronger password.');}
 ...
 if(input.action==='login-password'){
  let result;try{result=await api('/auth/v1/token?grant_type=password',{method:'POST',payload:{email:address,password:password(input.password,{existing:true}),...security}});}catch(e){providerError(e,'The email or password was not recognised, or the email still needs verification.');}
 ...
 if(input.action==='request-reset'){
  try{await api('/auth/v1/recover',{method:'POST',payload:{email:address,...security}});}catch(e){if(!['user_not_found','email_not_found'].includes(e.upstreamCode))providerError(e,'The reset request could not be sent. Complete a fresh security check and try again.');}
 ...
 if(input.action==='verify-code'||input.action==='verify-recovery'){
  const token=code(input.code);if(env.TURNSTILE_SITE_KEY||env.TURNSTILE_SECRET_KEY)await verifyBot(input.botToken,'nufi-account',env,fetcher);
